...
public class AvoidObstaclesMove : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        dir = GetAvoiddanceDirection(dir);//장애물이 없으면 그대로 / 있으면 변경된 방향이 적용
        ...
        Quaternion rot = Quaternion.LookRotation(dir);//방향을 알려주는 벡터를 넣으면 그 방향을 보는 값 설정
        transform.rotation = Quaternion.Slerp(transform.rotation, rot, steeringForce * Time.deltaTime);
        
    }

    //이동방향에 장애물이 있을 경우 이동하려는 방향을 바꾸는 기능
    public Vector3 GetAvoiddanceDirection(Vector3 dir)
    {
        RaycastHit hit;
        int layerMask = 1 << 15;
